...
onMount(async () => {
    ...
    async function updateServerStatus() {
        try {
            ...
            serverOnlinePlayers = status.players?.online || 0;
            serverOnline = status.online || false;
        } catch (e) {
            console.log("Failed to load server status");
            serverOnline = false;
        }
    }
    updateServerStatus();
            ...
            <div class="container-group">
                <div class="wrapper-group">
                    <div class="wrapper-item-group">
                        <i class="fa fa-server" class:text-success={serverOnline} class:text-danger={!serverOnline}></i> {serverOnlinePlayers} {$t("mainContent.serverPlayers")}
                    </div>
                </div>
                <div class="wrapper-group">
